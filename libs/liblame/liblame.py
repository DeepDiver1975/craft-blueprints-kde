...
if CraftCore.compiler.isGCCLike():
    ...
    class Package(MakeFilePackageBase):
        ...
        def install(self):
            return (
                utils.copyFile(os.path.join(self.sourceDir(), "include/lame.h"), os.path.join(self.installDir(), "include/lame/lame.h"), linkOnly=False)
                and utils.copyFile(os.path.join(self.sourceDir(), "output/lame_enc.dll"), os.path.join(self.installDir(), "bin/lame_enc.dll"), linkOnly=False)
                and utils.copyFile(os.path.join(self.sourceDir(), "output/lame_enc.lib"), os.path.join(self.installDir(), "lib/lame_enc.lib"), linkOnly=False)
                and utils.copyFile(
                    os.path.join(self.sourceDir(), "output/libmp3lame.dll"), os.path.join(self.installDir(), "bin/libmp3lame.dll"), linkOnly=False
                )
                and utils.copyFile(
                    os.path.join(self.sourceDir(), "output/libmp3lame.lib"), os.path.join(self.installDir(), "lib/libmp3lame.lib"), linkOnly=False
                )
            )
